...
for i in range(max_lines):
    ...
    # Correct alignment: force width
    output = f"{id_str:>3} {p_str:>2} {s_line:<{summary_width}} {type_str:<6} {b_line:<{branch_width}}"
    # Fix the i > 0 issue. The id_str is 3 wide, p_str is 2 wide, type is 6 wide
    if i > 0:
        # 3+1+2+1 = 7 spaces before summary starts
        # summary_width + 1 = spaces before type
        # 6 + 1 = spaces before branch
        output = f"{' ':>3} {' ':>2} {s_line:<{summary_width}} {' ':>6} {b_line:<{branch_width}}"
    print(f"'{output}'")
