...
for i in range(max_lines):
    ...
    id_p = f"{id_str:>3} {p_str:>2}"
    s_col = f"{s_line:<{summary_width}}"
    type_col = f"{type_str:<6}"
    b_col = f"{b_line:<{branch_width}}"
    
    # Corrected logic:
    if i == 0:
        print(f"{id_str:>3} {p_str:>2} {s_col} {type_col} {b_col}")
    else:
        # Wrap lines must have padding for id, p, type
        print(f"{'':>3} {'':>2} {s_col} {'':<6} {b_col}")
