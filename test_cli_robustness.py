...
class TestCLIRobustness(unittest.TestCase):
    ...
    def test_tasks_no_command(self):
        ...
        result = subprocess.run([sys.executable, self.tasks_py], 
    ...
    def test_repo_no_command(self):
        ...
        result = subprocess.run([sys.executable, self.repo_py], 
                               cwd=self.repo_dir, capture_output=True, text=True)
    ...
    def test_tasks_missing_tool(self):
        ...
        # Run it
        result = subprocess.run([sys.executable, self.tasks_py, "-j", "run", "lint"], 
                               cwd=self.repo_dir, capture_output=True, text=True)
    ...
    def test_repo_validation_failure(self):
        ...
        subprocess.run([sys.executable, self.tasks_py, "config", "set", "repo.lint", "missing-command"], 
                       cwd=self.repo_dir, capture_output=True)
        
        result = subprocess.run([sys.executable, self.repo_py, "commit", "test commit"], 
                               cwd=self.repo_dir, capture_output=True, text=True)
