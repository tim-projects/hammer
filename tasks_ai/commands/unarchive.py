...
def run(cli, task_id):
    ...
    if not branch:
        cli.error("TASK_METADATA_CORRUPTED", detail=f"Task {task_id} missing 'Br' metadata.")
        return

    # 3. Handle branch
    # Check if local
    local_branches = cli._run_git(["branch", "--format", "%(refname:short)"]).stdout.splitlines()
    ...
    with open(log_path, "a") as f:
        f.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ARCHIVED->PROGRESSING (UNARCHIVE)\n")
