...
class TasksCLI:
    ...
    def _generate_review_diff(self, task_path, branch):
        ...
        try:
            with open(debug_log2, "a") as f:
                f.write(
                    f"branch={branch}, default_branch={default_branch}, main_sha={main_sha}\n"
                )
        except (PermissionError, OSError):
            pass
    ...
    def init(self, force=False):
        def install_hooks(self):
            ...
            # Install post-checkout hook for stale branch detection
            post_checkout_path = os.path.join(hook_dir, "post-checkout")
            with open(post_checkout_path, "w") as f:
                f.write(
                    '#!/bin/bash\n\n# $1 = previous HEAD, $2 = new HEAD, $3 = flag (1 if branch checkout)\nif [ "$3" -eq 1 ]; then\n    branch=$(git rev-parse --abbrev-ref HEAD)\n    # Task branch pattern: <id>-<task-slug>\n    if [[ "$branch" =~ ^[0-9]+- ]]; then\n        echo "Checking staleness for branch: $branch"\n        main_branch=$(git rev-parse --abbrev-ref origin/main 2>/dev/null || echo "main")\n        # Check if local branch has commits not in main\n        ahead=$(git rev-list --count $main_branch..$branch)\n        if [ "$ahead" -eq 0 ]; then\n            echo "Branch is clean and up-to-date with $main_branch."\n        else\n            # Check for unstaged changes\n            status=$(git status --porcelain)\n            if [ -z "$status" ]; then\n                echo "Branch is $ahead commit(s) ahead of $main_branch. Attempting auto-sync..."\n                git merge $main_branch\n            else\n                echo "⚠️  Branch is $ahead commit(s) ahead of $main_branch and has local changes."\n                echo "Run \'git merge $main_branch\' when ready to handle potential conflicts."\n            fi\n        fi\n    fi\nfi'
                )
            os.chmod(post_checkout_path, 0o755)
    ...
    def _move_logic(self, filename, new_status, force=False, yes=False, sync=True):
        ...
        if (
            ...
            self.error(
                ...
                hint=(
                    hint
                    if current_state != "REVIEW" or new_status != "DONE"
                    else "To promote from REVIEW: 1) Run 'tasks audit <id>', 2) Run 'tasks modify <id> --regression-check', 3) Run 'tasks verify <id> --proof \"...\"', 4) Run 'tasks move <id> STAGING'. See 'tasks help' for command details."
                ),
            )
            ...
            for state, tasks in all_data.items():
                for t in tasks:
                    ...
                    for i in range(max_lines):
                        ...
                        status_f = (
                            f"{C_TYPE}{status_str:<7}{C_RESET}" if i == 0 else "       "
                        )
                        type_f = (
                            f"{C_TYPE}{type_str:<6}{C_RESET}" if i == 0 else "      "
                        )
    ...
    def doctor(self, fix=False):
        ...
        def create_bug_report(bug_id, title, repro, expected, actual):
            ...
            bug_path = os.path.join(log_dir, bug_filename)
...
"""
    ...
    def verify(self, task_id, proof):
        ...
        self.log(
            f"✅ Proof verified and criteria hash locked: {hasher.hexdigest()[:8]}..."
        )
    ...
    def audit(self, task_id):
        ...
        with open(audit_path, "w") as f:
            f.write(
                f"Audited by: {os.getlogin()}\nTime: {os.times()}\nPatch-Hash: {patch_hash}\n"
            )

        self.log(
            f"✅ Audit log created for task {filename} at {audit_path} (Hash: {patch_hash[:8]}...)"
        )
