...
for i in range(max_lines):
    id_str = str(t.get("id", "")) if i == 0 else " " * 3
    p_str = str(t["p"]) if i == 0 else " " * 2
    s_line = summary_lines[i] if i < len(summary_lines) else ""
    type_str = t["type"] if i == 0 else " " * 6
    b_line = branch_lines[i] if i < len(branch_lines) else ""
    
    # Use exact padding
    print(f"{id_str} {p_str} {s_line:<{summary_width}} {type_str} {b_line:<{branch_width}}")
